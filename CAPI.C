...
char* capi_get_key_string(char* certID) {
    ...
    sprintf(keystring, "ssh-rsa %s", key64);
cleanup:
	return keystring;
}
...
void capi_release_key(struct capi_keyhandle_struct** keyhandle) {
    if (keyhandle) {
        if (*keyhandle) {
            ...
            {
                if (CERT_NCRYPT_KEY_SPEC == (*keyhandle)->win_keyspec)
                {
                    NCryptFreeObject((NCRYPT_HANDLE) (*keyhandle)->win_provider);
                }
                else
                {
...
BOOL capi_get_key_handle(void *f, char* certID, struct capi_keyhandle_struct** keyhandle) {
	...
	*keyhandle = NULL;
    ...
    if (!CryptAcquireCertificatePrivateKey(
        ...
        NULL,
        (HCRYPTPROV_OR_NCRYPT_KEY_HANDLE *) &newkeyhandle->win_provider,
        &newkeyhandle->win_keyspec,
        &fFreeContext))
    {
        debuglog("capi_get_key_handle: Error calling CryptAcquireCertificatePrivateKey. GetLastError()=%i (0x%08x)\n", GetLastError(), GetLastError());
        goto cleanup;
    }
    /*
...
cleanup:
    ...
    */
	if (!retval) {
        capi_release_key(&newkeyhandle);
	}

	return retval;
}

unsigned char* ssh_style_sig_from_crypto_handle(
    HCRYPTPROV_OR_NCRYPT_KEY_HANDLE hProvOrKey,
    DWORD dwKeySpec,
    char *sigdata,
    int sigdata_len,
    int *sigblob_len)
{
    ...
    BYTE rgbHash[SHA1_BYTES];
    BCRYPT_ALG_HANDLE hAlg = NULL;
    BCRYPT_HASH_HANDLE hHash = NULL;
    DWORD cbHashObject = 0, cbData = 0;
    PBYTE pbHashObject = NULL;
    unsigned char* retval = NULL, *rawsig = NULL, *p = NULL;
    HCRYPTHASH hash = 0;
    DWORD tmpSize = 0, rawsig_len = 0, x = 0;
    BYTE b = 0;
    Bignum bn = NULL;
    ...
    if (CERT_NCRYPT_KEY_SPEC == dwKeySpec)
    {
        ...
        if (NULL == (pbHashObject = malloc(cbHashObject)))
        {
            goto cleanup;
        }
        ...
        if (NULL == (rawsig = malloc(rawsig_len)))
            goto cleanup;
        ...
        // CNG uses reverse byte ordering of CAPI
        for (x = 0; x < rawsig_len / 2; x++)
        {
            b = rawsig[x];
            rawsig[x] = rawsig[rawsig_len - x - 1];
            rawsig[rawsig_len - x - 1] = b;
        }
    }
    else
    {
        // create the hash object, set it to SHA-1 and confirm expectations
        if (!CryptCreateHash(hProvOrKey, CALG_SHA1, 0, 0, &hash))
            goto cleanup;
        if (!CryptHashData(hash, sigdata, sigdata_len, 0))
            goto cleanup;

        if (!CryptSignHash(hash, dwKeySpec, NULL, 0, NULL, &rawsig_len))
            goto cleanup;
        if (NULL == (rawsig = malloc(rawsig_len)))
            goto cleanup;
        if (!CryptSignHash(hash, dwKeySpec, NULL, 0, rawsig, &rawsig_len))
            goto cleanup;
    }
...
unsigned char* capi_sig_certID(char* certID, char *sigdata, int sigdata_len, int *sigblob_len) {
	BOOL success = FALSE;
    HCRYPTPROV_OR_NCRYPT_KEY_HANDLE hProvOrKey = 0;
    DWORD dwKeySpec = 0;
    BOOL fFreeContext = FALSE;
    ...
    if (NULL == (retval = ssh_style_sig_from_crypto_handle(
        hProvOrKey,
        dwKeySpec,
        sigdata,
        sigdata_len,
...
unsigned char* capi_sig(struct capi_keyhandle_struct* keyhandle, char *sigdata, int sigdata_len, int *sigblob_len) {
    ...
    if (NULL == (retval = ssh_style_sig_from_crypto_handle(
        (HCRYPTPROV_OR_NCRYPT_KEY_HANDLE) keyhandle->win_provider,
        keyhandle->win_keyspec,
        sigdata,
        sigdata_len,
        sigblob_len)))
    {
...
struct CAPI_userkey* Create_CAPI_userkey(const char* certID, PCCERT_CONTEXT pCertContext) {
	BOOL success = FALSE;
	struct CAPI_userkey* retval = NULL;
	PCCERT_CONTEXT LpCertContext = NULL;

	if (pCertContext == NULL) {
		if (!capi_get_cert_handle((char *) certID, &LpCertContext)) {
